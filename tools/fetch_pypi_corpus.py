...
def is_clean_requirement(text):
    """Like `is_clean_string`, but double quotes are allowed.

    PEP 508 markers quote their values (`python_version < "3.10"`), and the
    MoonBit fixture escapes quotes on the way out, so they are safe to keep.
    Backslashes and control characters stay forbidden.
    """
    if not isinstance(text, str) or not text:
        return False
    if not text.isascii():
...
MBT_REQUIREMENTS_DOC = [
    "Real-world `Requires-Dist` entries, kept verbatim: this is the input that",
    "`Requirement::parse` has to handle, including extras, `@ url` forms and",
    "environment markers. Deduplicated and sorted; %d of them." % REQUIREMENT_CAP,
    "Regenerate with tools/fetch_pypi_corpus.py; do not edit by hand.",
]
MBT_FILENAMES_DOC = [
    "Real-world distribution filenames from the PyPI JSON API (`urls[].filename`):",
    "wheels and source distributions that `parse_wheel_filename` and",
    "`parse_sdist_filename` have to handle, including build tags, local version",
    "segments and compressed tag sets. Deduplicated and sorted; %d of them."
    % FILENAME_CAP,
    "Regenerate with tools/fetch_pypi_corpus.py; do not edit by hand.",
]


def render_mbt(versions, specifiers, requirements, filenames):
    blocks = [
        render_mbt_array("pypi_versions", MBT_VERSIONS_DOC, versions),
        render_mbt_array("pypi_specifiers", MBT_SPECIFIERS_DOC, specifiers),
        render_mbt_array("pypi_requires_dist", MBT_REQUIREMENTS_DOC, requirements),
        render_mbt_array("pypi_filenames", MBT_FILENAMES_DOC, filenames),
    ]
    return "\n\n".join(blocks) + "\n"
...
def main():
    ...
    def bump_requirement(reason):
        dropped_requirements[reason] = dropped_requirements.get(reason, 0) + 1
    ...
    for package in PACKAGES:
        ...
        # Full requirement lines, verbatim: the input for Requirement::parse.
        for entry in info.get("requires_dist") or []:
            if not isinstance(entry, str) or not is_clean_requirement(entry):
                continue
            if HAVE_PACKAGING:
                try:
                    Requirement(entry)
                except Exception:
                    bump_requirement("rejected by packaging.Requirement")
                    continue
            raw_requirements.add(entry)

        # Distribution filenames: the input for the filename parsers.
        for file_info in payload.get("urls") or []:
            if not isinstance(file_info, dict):
                continue
            name = file_info.get("filename")
            if not isinstance(name, str) or not is_clean_string(name):
                continue
            if not (name.endswith(".whl") or name.endswith(".tar.gz") or name.endswith(".zip")):
                continue
            raw_filenames.add(name)
    ...
    requirements_out = sorted(raw_requirements)[:REQUIREMENT_CAP]
    filenames_out = sorted(raw_filenames)[:FILENAME_CAP]

    generated_note = (
        "Generated by tools/fetch_pypi_corpus.py on %s from the PyPI JSON API "
        "(<index>/<package>/json: `releases`, `info.requires_python`, "
        "`info.requires_dist`, `urls[].filename`); index base = %s. "
        "Version specifiers and requirement lines were validated with "
    ...
    corpus = {
        ...
        "requirement_count": len(requirements_out),
        "filename_count": len(filenames_out),
    ...
    with open(MBT_OUT, "w", encoding="utf-8") as handle:
        handle.write(
            render_mbt(versions_out, specifiers_out, requirements_out, filenames_out)
        )
    ...
    print(
        "requirement lines        : %d (cap %d)" % (len(requirements_out), REQUIREMENT_CAP)
    )
    print("distribution filenames   : %d (cap %d)" % (len(filenames_out), FILENAME_CAP))
    ...
    for reason, count in sorted(dropped_requirements.items()):
        print("dropped requirement lines: %d (%s)" % (count, reason))
