...
> 2026-09-16 更新：本文件列出的工作项**已全部落地**（M1–M13，含场景 3 的升级短名单），
> 当前模块版本 0.2.0。最新申报事实以 `docs/proposal.md`、`README.md` 和
> `tools/source_metrics.py` 为准；下文保留施工前的 `[已有]` / `[计划]` 记录，用于说明
> 范围如何扩展，**不应再作为当前状态直接引用**，其中的 `[计划]` 也不再表示"待办"。

> 本文件是**工作项清单**，沿用 [项目申报书](proposal.md) 的章节结构，说明"还要做什么、
> 做到什么算完成、怎么验收"。它不是申报书，也不直接提交。
>
> 现状与目标的区别在每节里显式标注：`[已有]` = 已实现并有验收证据，
> `[计划]` = 当时尚未实现（**本文所有 `[计划]` 项现已落地**）。这条约定当初是用来防止
> "申报书承诺了库做不到的事"的，现在反过来提醒：改写 `proposal.md` 时只写有证据的部分。
...
字符串验证过（见下方"验收证据"列）。**实际交付了四个**：除下面的场景 1、2、3 之外，
`examples/audit`（跨制品审计）也是独立可运行的产物，见"交付成果"一节。
...
`[已完成，与上面的设想不同]` 实际交付的是 `upgrade.mbt` 的 `upgrade_shortlist` 与
`examples/upgrade-check`：输入当前版本、目标区间、预发布策略、目标解释器与候选列表，
输出升序短名单与每个被弃候选的原因码。**没有**做"对比两个版本的依赖声明找约束变严"——
那需要解析多份 `METADATA` 再逐条比约束，属于调用方的事；库这一层只回答版本问题，
并把"短名单不等于可以安全升级"打印在自己的输出里。51 条语料 + 3 条不可判定输入。
...
`[计划]` 到 `[已完成]`：下面这些当初列在"还差"里，现已全部交付并在 CI 中逐后端比对：
...
- **不探测宿主**的平台兼容性标签（标签的**计算**已在 M12 做完，此处指不去问运行中的机器）：
  `parse_wheel_filename` 解析出 tag，
...
- 不评估升级的安全性或 API 兼容性：`upgrade_shortlist`（M13）只回答版本问题，并把这条
  边界打印在 `examples/upgrade-check` 自己的输出里；
