...
> 生成 fixture 和 Python 工具的透明口径，当前根目录生产 MoonBit 为 9,778 行；另有
> 7,311 行 MoonBit 测试、293 个测试块。该结果可在仓库根目录直接复核。
>
> 本次同时增加实际集成场景，共四个可运行产物。`examples/upgrade-check` 读入当前版本、
> 目标区间与候选列表，输出待测短名单与每个被弃候选的原因码，并显式声明「满足版本约束
> 不等于可以安全升级」。`examples/audit` 使用真实 PyPI Flask 0.12.5 的 PEP 658
...
>
> 可靠性方面，293 个测试覆盖 wasm、wasm-gc、js、native；真实语料差分共 122,640 条，
> 固定对照 `packaging==26.3`，并保留 mutation probe（43 处注入缺陷全部检出）证明测试能够发现故意注入的
...
- 实际场景：`docs/audit-scenario.md`、`docs/upgrade-scenario.md`
