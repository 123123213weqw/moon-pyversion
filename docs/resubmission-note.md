...
> 生成 fixture 和 Python 工具的透明口径，当前根目录生产 MoonBit 为 8,976 行；另有
> 6,605 行 MoonBit 测试、259 个测试块。该结果可在仓库根目录直接复核。
>
...
>
> 可靠性方面，259 个测试覆盖 wasm、wasm-gc、js、native；真实语料差分共 122,507 条，
> 固定对照 `packaging==26.3`，并保留 mutation probe 证明测试能够发现故意注入的缺陷。
