...
- `packaging` 26.3：版本、约束、分发文件名、需求行、环境标记、许可证表达式与
  核心元数据的黑盒对照，仅在测试与 `tools/` 中调用；
- `tomli` 2.4.1：TOML 1.0 文档裁决的对照读取器（harness 找不到时回落到标准库
  `tomllib`，并把实际使用的读取器写进报告）；
- `uv` 0.11.32：用 `uv export --format pylock.toml` 写出 6 份真实锁文件
  （`pylock_cases/uv/`）。`uv` 只写不读、不给出裁决，因此它提供的是**输入的独立
  性**，不是 PEP 751 的第二份读法；
- 上述三者都是测试期工具，库的运行时依赖仍然只有 `moonbitlang/core`。
