...
- MoonBit 模块：`123123213weqw/moon_pyversion`，仓库版本 0.2.0，许可证 Apache-2.0；
  MoonCakes 上已发布 **0.1.0**（注册表索引中唯一的版本，未 yank，归档 sha256
  `16379fad…358fd`），含本轮扩展的 **0.2.0 尚未发布**
...
> 数据以仓库当前提交为准；提交前核对最新提交 SHA 与 CI 结论。0.1.0 的下载、安装与运行
> 已在 `docs/release-verification.md` 中记录（含归档 sha256 与注册表索引记录）；0.2.0 发布
> 后必须按同一份步骤重跑一次，再更新本稿的发布状态。
